...
def train(
    ...
    token_ids = [item.token_id for item in filteredDataset] 
    ...
    for i, (corpus_ids, corpus_mask, label_ids) in progress:
        ...
        argmax_indices = probabilities.argmax(dim=-1)
        predictions = [token_ids[index] if index < len(token_ids) else token_ids[-1] for index in argmax_indices]
